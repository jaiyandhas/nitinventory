...
4. **Audit-Compliant PO Cancellations & PR Cloning**
   - Implemented PO cancellation which triggers automatic database-level balance reversals (refunding `deducted_amount` back to the budget).
   - Stored cancellation reasons in the `po_cancellations` audit table.
   - Restricted request cancellation to *only* when the Purchase Order (PO) has been issued (cancellation during active tendering or approval phases is disabled).
...
6. **Budget Master Initiation, Technical Committee, Consultation & Formatting**
   - **Standalone Budget Initiation**: Created a full page form (`BudgetFormPage.tsx`) supporting live, strictly autogenerated budget file reference numbers rolling like `NITT/F.No.0000/CAPEX/2026-27/CSE`.
   - **Sources of Fund (Migration)**: Migrated "expenditure category" to "source of fund" globally across database columns, backend models, and settings keys.
   - **Indian Currency Formatting**: Removed all Lakhs formatting/shorthands (`₹XX.XXL`) in favor of showing raw integers/decimals formatted with proper Indian currency commas (e.g. `₹1,50,000.00`). Implemented custom Jinja2 filter `format_inr` for print PDF templates and a central `formatCurrency` utility on the React frontend.
   - **Editable Categories**: Managed budget categories dynamically, allowing on-the-fly custom category registration.
...
- **`test_cancellation.py`**: Ensures PO cancellation refunds the allocated budget and cloning re-initiation tracks parent PR IDs, while verifying that pre-PO tender cancellation is disabled.
