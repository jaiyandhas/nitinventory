...
export const PRDetailPage: React.FC = () => {
  ...
  const quotationDoc = pr.documents?.find((d: any) => d.doc_key === 'quotation_file');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="card p-6 grid grid-cols-2 gap-6">
            ...
            {((pr.emd !== undefined && pr.emd !== null) || (pr.performance_security !== undefined && pr.performance_security !== null)) && (
              <div className="col-span-2 border-t border-slate-100 pt-4 grid grid-cols-2 gap-4">
            ...
            {/* Split & Exemption justification */}
            {(pr.is_item_split || pr.is_quantity_split || pr.exemption) && (
              <div className="col-span-2 border-t border-slate-100 pt-4 space-y-3">
                <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Splitting & Exemption Details</div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {(pr.is_item_split || pr.is_quantity_split) && (
                    <div className="text-sm text-slate-700 bg-slate-50 p-2.5 border border-slate-200 rounded space-y-1">
                      {pr.is_item_split && <div><span className="font-bold">Item Split Justification:</span> {pr.item_split_justification || 'Yes'}</div>}
                      {pr.is_quantity_split && <div><span className="font-bold">Quantity Split Justification:</span> {pr.quantity_split_details || 'Yes'}</div>}
                    </div>
                  )}
                  {pr.exemption && (
                    <div className="text-sm text-slate-700 bg-orange-50/50 p-2.5 border border-orange-100 rounded">
                      <span className="font-bold text-orange-800">Proprietary / Exemption Route:</span> {pr.exemption_remarks || 'No remarks provided'}
                    </div>
                  )}
                </div>
              </div>
            ...
            {/* Delivery, Southern Region Service Center & Training Requirements */}
            <div className="col-span-2 border-t border-slate-100 pt-4 grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-slate-700">
              {/* Delivery Details */}
              {(pr.delivery_location || pr.delivery_mode) && (
                <div className="space-y-2">
                  <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Delivery Details</div>
                  <div className="bg-slate-50 p-2.5 border border-slate-200 rounded space-y-1">
                    {pr.delivery_location && <div><span className="font-bold text-slate-600">Location:</span> {pr.delivery_location}</div>}
                    {pr.delivery_mode && <div><span className="font-bold text-slate-600">Mode:</span> {pr.delivery_mode}</div>}
                  </div>
                </div>
              )}
              ...
              <div className="space-y-2">
                <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Southern Region Service Centre</div>
                <div className="bg-slate-50 p-2.5 border border-slate-200 rounded">
                  {pr.is_service_center_in_south ? (
                    <div>
                      <span className="font-bold text-green-700">Yes</span>
                      {pr.service_center_south_desc && <p className="text-xs text-slate-600 mt-1 italic whitespace-pre-line">{pr.service_center_south_desc}</p>}
                    </div>
                  ) : (
              ...
              <div className="md:col-span-2 space-y-2">
                <div className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Training Requirements</div>
                <div className="bg-slate-50 p-2.5 border border-slate-200 rounded font-medium">
                  {pr.is_training_required ? (
                    <div className="space-y-1 text-xs">
                      <div><span className="font-bold text-blue-700">Required:</span> Yes</div>
                      {pr.training_type && <div><span className="font-bold text-slate-600">Training Type:</span> {pr.training_type}</div>}
                      {pr.training_vendor && <div><span className="font-bold text-slate-600">Training Vendor:</span> {pr.training_vendor}</div>}
                      {pr.training_comments && <div><span className="font-bold text-slate-600">Additional Comments:</span> {pr.training_comments}</div>}
                    </div>
                  ) : (
                    <span className="text-slate-600">No training required</span>
                  )}
                </div>
              </div>
            </div>

            {/* Basis of Estimate & quotation_file */}
            {pr.basis_of_estimate && (
              <div className="col-span-2 border-t border-slate-100 pt-4 space-y-2">
                ...
                <div className="bg-slate-50 p-3 border border-slate-200 rounded space-y-2">
                  <p className="text-sm text-slate-700">{pr.basis_of_estimate}</p>
                  {quotationDoc && (
                    <div className="pt-2 border-t border-slate-200/60">
                      <span className="text-xs font-bold text-slate-500 block mb-1">Attached Basis of Estimate PDF</span>
                      <a 
                        href={quotationDoc.path} 
                        target="_blank" 
                        rel="noreferrer" 
                        className="inline-flex items-center gap-1.5 bg-white border border-slate-200 hover:bg-slate-50 text-[#1a3a6b] px-3 py-1.5 rounded text-xs font-semibold shadow-sm transition"
                      >
                        <FileText size={14} className="text-blue-500" />
                        <span>{quotationDoc.original_name}</span>
            ...
            <div className="card">
              ...
              <div className="divide-y divide-slate-200">
                {pr.items.map((item, idx) => {
                  const techSpecDoc = pr.documents?.find((d: any) => d.doc_key === `item_${idx}_tech_spec`);
                  const gemNacDoc = pr.documents?.find((d: any) => d.doc_key === `item_${idx}_gem_nac`);
                  return (
                        ...
                        <div>
                          ...
                          <div className="mt-1 flex flex-wrap gap-2 text-xs">
                            <span className="bg-slate-100 text-slate-600 px-2 py-0.5 rounded font-semibold">
                              Requirement: {item.requirement_type || 'N/A'}
                            </span>
                            {item.charges !== undefined && item.charges !== null && (
                              <span className="bg-blue-50 text-blue-700 px-2 py-0.5 rounded font-semibold">
                                GST/Charges: {item.charges}%
                              </span>
                            )}
                          </div>
                        </div>
                        <span className="text-sm font-bold text-[#1a3a6b] whitespace-nowrap">
                          {formatCurrency(item.estimated_total)}
                        </span>
                      </div>

                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50/50 p-4 border border-slate-150 rounded text-xs text-slate-700 font-medium">
                        <div>
                          <span className="font-bold text-slate-500 block mb-0.5">Warranty & Delivery</span>
                          <p>
                            Warranty: {item.warranty !== undefined && item.warranty !== null ? `${item.warranty} Months` : 'N/A'} · 
                            Delivery Period: {item.delivery_period !== undefined && item.delivery_period !== null ? `${item.delivery_period} Weeks` : 'N/A'}
                          </p>
                        </div>

                        <div>
                          <span className="font-bold text-slate-500 block mb-0.5">Installation</span>
                          <p>
                            {item.installation_required ? 'Required' : 'Not Required'}
                          </p>
                        </div>

                        <div>
                          <span className="font-bold text-slate-500 block mb-0.5">Site Readiness</span>
                          <p>
                            {item.site_readiness ? 'Ready' : `Not Ready (${item.site_readiness_remarks || 'No remarks'})`}
                          </p>
                        </div>

                        <div>
                          <span className="font-bold text-slate-500 block mb-0.5">Departmental Availability</span>
                          <p>
                            {item.availability === 'Yes' 
                              ? `Available in Dept (Stock: ${item.present_stock || '0'}, File Ref: ${item.previous_file_no_reference || 'N/A'})` 
                              : 'Not Available in Dept'}
                          </p>
                          {item.availability === 'Yes' && item.justification_for_procurement && (
                            <p className="text-[11px] text-slate-500 mt-1 italic font-normal">
                              Justification: "{item.justification_for_procurement}"
                            </p>
                          )}
                        </div>
                        ...
                        {item.tech_specs_text && (
                          <div className="md:col-span-2">
                            <span className="font-bold text-slate-500 block mb-0.5">Technical Specifications / Brief</span>
                            <p className="bg-white p-2.5 border border-slate-100 rounded text-slate-600 font-mono whitespace-pre-wrap font-normal">
                              {item.tech_specs_text}
                            </p>
                          </div>
                        )}
                        ...
                        {item.gem_link && (
                          <div className="md:col-span-2">
                            <span className="font-bold text-slate-500 block mb-0.5">GeM Link / Details</span>
                            <a href={item.gem_link} target="_blank" rel="noreferrer" className="text-blue-600 hover:underline font-bold">
                              {item.gem_link}
                            </a>
                          </div>
                        )}

                        {(techSpecDoc || gemNacDoc) && (
                          <div className="md:col-span-2 border-t border-slate-200/60 pt-3 mt-1 space-y-2">
                            <span className="font-bold text-slate-500 block">Attached Documents</span>
                            <div className="flex flex-wrap gap-3">
                              {techSpecDoc && (
                                <a 
                                  href={techSpecDoc.path} 
                                  target="_blank" 
                                  rel="noreferrer" 
                                  className="flex items-center gap-1.5 bg-white border border-slate-200 hover:bg-slate-50 text-[#1a3a6b] px-3 py-1.5 rounded text-xs font-semibold shadow-sm transition animate-fadeIn"
                                >
                                  <FileText size={14} className="text-red-500" />
                                  <span>Tech Spec PDF ({techSpecDoc.original_name})</span>
                                  <Download size={12} className="text-slate-400 ml-0.5" />
                                </a>
                              )}
                              {gemNacDoc && (
                                ...
                                >
                                  ...
                                  <span>GeM NAC PDF ({gemNacDoc.original_name})</span>
                                  <Download size={12} className="text-slate-400 ml-0.5" />
                                </a>
                              )}
                            </div>
