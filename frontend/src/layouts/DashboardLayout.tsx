...
const NAV_ITEMS: NavItem[] = [
  ...
  { label: "Purchase Indents", icon: FileText, href: "/pr" },
  {
    ...
    icon: Wallet,
    href: "/budget",
    roles: ["faculty", "hod", "admin", "dean_approver", "apex_approver"],
  },
  {
    label: "Deliveries",
    icon: Truck,
    href: "/inventory/deliveries",
    roles: ["faculty", "hod", "verifier_sp", "admin"],
  },
  { label: "Assets", icon: Box, href: "/assets" },
  {
    ...
    roles: ["admin", "verifier_sp", "apex_approver"],
  },
  {
    label: "Analytics",
    icon: BarChart2,
    href: "/analytics",
    roles: ["admin", "apex_approver"],
  },
...
export const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({
  ...
  const visibleItems = NAV_ITEMS.filter((item) => {
    if (
      item.label === "Purchase Indents" &&
      user?.designation === "Dean P&D (Budget)"
    ) {
      return false;
    }
    return (
      !item.roles || (user?.role && item.roles.includes(user.role.group_key))
    );
  });
  ...
  const Sidebar = ({ mobile = false }: { mobile?: boolean }) => (
    ...
    >
      {/* Branding */}
      <div
        className={`flex items-center gap-3 px-4 py-5 border-b border-slate-300 bg-white ${collapsed && !mobile ? "justify-center" : ""}`}
      >
        <img
          src="/NITLOGO.png"
          alt="NIT Logo"
        ...
        {(!collapsed || mobile) && (
          <div>
            <div className="text-lg font-black text-[#1a3a6b] tracking-tight leading-none">
              STORES and PURCHASE MANAGEMENT
            </div>
            <div className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mt-1.5">
      ...
      {/* User footer */}
      <div
        className={`border-t border-slate-300 p-3 bg-white ${collapsed && !mobile ? "flex flex-col items-center gap-2" : ""}`}
      >
        {!collapsed || mobile ? (
          <div className="flex items-center gap-3 w-full">
            <Link
              to="/profile"
              className="w-8 h-8 bg-blue-100 border border-blue-200 flex items-center justify-center text-xs font-bold text-[#1a3a6b] flex-shrink-0 hover:bg-blue-200 transition-colors rounded-sm"
            ...
            <div className="flex-1 min-w-0">
              <Link
                to="/profile"
                className="text-xs font-bold text-slate-800 truncate hover:text-[#1a3a6b] hover:underline block"
              >
                {user?.name}
              </Link>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="flex items-center justify-between px-6 py-4 bg-white border-b border-slate-300 shadow-sm flex-shrink-0 z-10">
          <div className="flex items-center gap-3">
            <button
              className="md:hidden text-slate-600 hover:text-[#1a3a6b]"
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Institutional Resource & Inventory System
              </p>
            </div>
          </div>
        ...
        {user && !user.signature_path && location.pathname !== "/profile" && (
          <div className="flex items-center gap-3 px-6 py-2.5 bg-amber-50 border-b border-amber-200 text-amber-900 flex-shrink-0">
            ...
            <p className="text-xs font-semibold flex-1">
              ...
              Your account has no digital signature uploaded. Workflow approvals
              and official documents require your signature.
            </p>
            <Link
              to="/profile"
